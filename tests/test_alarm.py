...
def test_alarm_set_get():
    ...
    appid = "appid2"
    mgr.set_application_id(appid)
    assert mgr.application_id == appid
    moid = "moid2"
    mgr.set_managed_object_id(moid)
    assert mgr.managed_object_id == moid
