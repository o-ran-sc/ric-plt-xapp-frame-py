...
# Use camel case to comply with the Adapter JSON requirements.
KEY_ALARM = "alarm"
...
class AlarmManager:
    """
    Provides an API for an Xapp to raise and clear alarms by sending messages to the Alarm Adapter.
    ...
    def set_managed_object_id(self, managed_object_id: str):
        """
        Parameters
        ----------
        managed_object_id: str
    ...
    def _create_alarm_message(alarm: AlarmDetail, action: AlarmAction):
        """
        Creates a dict with the specified alarm detail and action.
        Uses the current system time, measured in milliseconds since the Epoch.
        The keys are camel case to meet Alarm Adapter requirement.
