...
class _BaseXapp:
    ...
    def _do_post(self, url, msg, num_of_retries=5):
        ...
        with requests.Session() as session:
            ...
            return session.post(url, json=msg)
    ...
    def register(self):
        ...
        try:
            ...
            resp = self._do_post(reg_url, request_string)
            if resp.status_code == 200 or resp.status_code == 201:
                self.logger.info('Xapp registered')
                self._is_registered = True
                return

            raise XappError(f"Response with status code: {resp.status_code}, message: {resp.text}")
        except Exception as e:
            raise XappError(f"Registration failed due to {str(e)}") from e
    ...
    def deregister(self):
        ...
        try:
            self.logger.debug(f'De-registration endpoint: {dereg_url}, request: {request_string}')

            resp = self._do_post(dereg_url, request_string)
            ...
            raise XappError(f"De-registration failed due to {str(e)}") from e
