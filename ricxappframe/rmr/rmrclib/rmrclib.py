...
# Private subpackage that creates and publishes a reference to the C shared
# libray and objects initialized at start by calling the C shared library.
...
# publish constants from the library
RMR_MAX_RCV_BYTES = get_constants().get("RMR_MAX_RCV_BYTES")
RMRFL_MTCALL = get_constants().get("RMRFL_MTCALL", 0x02)  # initialization flags
RMRFL_NONE = get_constants().get("RMRFL_NONE", 0x0)
RMR_OK = get_constants().get("RMR_OK")
RMR_ERR_TIMEOUT = get_constants().get("RMR_ERR_TIMEOUT")
RMR_ERR_RETRY = get_constants().get("RMR_ERR_RETRY")
