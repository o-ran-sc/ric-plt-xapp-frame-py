...
from ricxappframe.xapp_frame import RMRXapp, XAPP_DESCRIPTOR_ENV
...
def test_config_no_env(monkeypatch):
    init_config_file()
    monkeypatch.delenv(XAPP_DESCRIPTOR_ENV, raising=False)
...
def test_default_config_handler(monkeypatch):
    """Just for coverage"""
    init_config_file()
    monkeypatch.setenv(XAPP_DESCRIPTOR_ENV, config_file_path)
