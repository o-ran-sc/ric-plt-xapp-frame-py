...
class MetricsManager:
    ...
    def send_report(self, msg: MetricsReport):
        ...
        """
        if len(msg[KEY_DATA]) == 0:
            raise EmptyReport
        payload = json.dumps(msg).encode()
        mdc_logger.debug("_rmr_send_metrics: payload is {}".format(payload))
        ...
        for _ in range(0, RETRIES):
            ...
            mdc_logger.debug("_rmr_send_metrics: try {0} result is {1}".format(_, post_send_summary[rmr.RMR_MS_MSG_STATE]))
        ...
        if post_send_summary[rmr.RMR_MS_MSG_STATE] != rmr.RMR_OK:
            mdc_logger.warning("_rmr_send_metrics: failed after {} retries".format(RETRIES))
            return False
