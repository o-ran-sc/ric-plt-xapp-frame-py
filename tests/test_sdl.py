...
def test_sdl_set_get():
    ...
    assert sdl.delete_if(NS, "gs.df4", "delete") == False
    assert sdl.delete_if(NS, "gs.df4", "delete_this") == True
...
def test_sdl_member():
    ...
    assert sdl.is_member(NS, "group1", "member1") == True

    sdl.remove_member(NS, "group1", "not_member")
    assert sdl.is_member(NS, "group1", "member1") == True

    sdl.remove_member(NS, "group1", "member1")
    assert sdl.is_member(NS, "group1", "member1") == False
...
def test_sdl_set_and_publishs_with_handle_events():
    """
...
def test_sdl_remove_and_publishs_with_start_event_listener():
    """
